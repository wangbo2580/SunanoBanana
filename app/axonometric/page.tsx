...
import { AxonometricConfigPanel } from "@/components/axonometric/axonometric-config"
import { buildPrompt } from "@/lib/prompt-templates"
import type { AxonometricConfig } from "@/lib/prompt-templates"

export default function AxonometricPage() {
  const [imageUrl, setImageUrl] = useState<string | null>(null)
  const [config, setConfig] = useState<AxonometricConfig>({
    language: "bilingual",
  })
  ...
  const handleGenerate = async () => {
    if (!imageUrl) return
    await generate(imageUrl, buildPrompt("axonometric", config))
  }
    ...
    <div className="min-h-screen bg-muted/20">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2">3D轴测图</h1>
            <p className="text-muted-foreground">以3D立体轴测爆炸图展示家具搭配关系与材质配色</p>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <Card className="p-6">
                ...
                <UploadZone value={imageUrl} onChange={(url) => { setImageUrl(url); reset() }} label="上传室内场景照片" hint="建议上传家具完整可见的照片" />
              </Card>

              <Card className="p-6">
                <h3 className="font-semibold mb-4">生成配置</h3>
                <AxonometricConfigPanel value={config} onChange={setConfig} />
              </Card>
            ...
            <div className="lg:col-span-3">
              <Card className="p-6">
                ...
                <ResultDisplay status={status} originalImage={imageUrl} resultImage={resultImage} error={error} onRetry={handleGenerate} onDownload={() => { if (resultImage) { const a = document.createElement("a"); a.href = resultImage; a.download = `banana-axonometric-${Date.now()}.png`; a.target = "_blank"; a.click(); } }} />
              </Card>
            </div>
          </div>
        </div>
