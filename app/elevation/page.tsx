...
import { ElevationConfigPanel } from "@/components/elevation/elevation-config"
import { buildPrompt } from "@/lib/prompt-templates"
import type { ElevationConfig } from "@/lib/prompt-templates"

export default function ElevationPage() {
  const [imageUrl, setImageUrl] = useState<string | null>(null)
  const [config, setConfig] = useState<ElevationConfig>({
    style: "revit",
    language: "bilingual",
  })
  ...
  const handleGenerate = async () => {
    if (!imageUrl) return
    await generate(imageUrl, buildPrompt("elevation", config))
  }
    ...
    <div className="min-h-screen bg-muted/20">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2">立面产品图</h1>
            <p className="text-muted-foreground">从室内照片中提取家具，生成带尺寸标注的专业立面图纸</p>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card className="p-6">
                ...
                <ElevationConfigPanel value={config} onChange={setConfig} />
              </Card>
            ...
            <div className="lg:col-span-3">
              <Card className="p-6">
                ...
                <ResultDisplay status={status} originalImage={imageUrl} resultImage={resultImage} error={error} onRetry={handleGenerate} onDownload={() => { if (resultImage) { const a = document.createElement("a"); a.href = resultImage; a.download = `banana-elevation-${Date.now()}.png`; a.target = "_blank"; a.click(); } }} />
              </Card>
            </div>
          </div>
        </div>
