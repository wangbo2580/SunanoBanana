...
import { SpatialConfigPanel } from "@/components/spatial-analysis/spatial-config"
...
export default function SpatialAnalysisPage() {
  ...
  const [config, setConfig] = useState<SpatialAnalysisConfig>({
    language: "bilingual",
  })
  ...
  const handleGenerate = async () => {
    if (!imageUrl) return
    await generate(imageUrl, buildPrompt("spatial-analysis", config))
  }
    ...
    <div className="min-h-screen bg-muted/20">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2">尺度分析</h1>
            <p className="text-muted-foreground">生成空间尺度标注、人体工学示意与家具配置分析图</p>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <Card className="p-6">
                ...
                <UploadZone value={imageUrl} onChange={(url) => { setImageUrl(url); reset() }} label="上传平面户型图" hint="建议上传带家具布置的平面图" />
              </Card>

              <Card className="p-6">
                <h3 className="font-semibold mb-4">生成配置</h3>
                <SpatialConfigPanel value={config} onChange={setConfig} />
              </Card>
            ...
            <div className="lg:col-span-3">
              <Card className="p-6">
                ...
                <ResultDisplay status={status} originalImage={imageUrl} resultImage={resultImage} error={error} onRetry={handleGenerate} onDownload={() => { if (resultImage) { const a = document.createElement("a"); a.href = resultImage; a.download = `banana-spatial-${Date.now()}.png`; a.target = "_blank"; a.click(); } }} />
              </Card>
            </div>
          </div>
        </div>
