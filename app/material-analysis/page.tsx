...
import { MaterialConfigPanel } from "@/components/material-analysis/material-config"
...
export default function MaterialAnalysisPage() {
  ...
  const [config, setConfig] = useState<MaterialAnalysisConfig>({
    language: "bilingual",
  })
  ...
  const handleGenerate = async () => {
    if (!imageUrl) return
    await generate(imageUrl, buildPrompt("material-analysis", config))
  }
    ...
    <div className="min-h-screen bg-muted/20">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2">家具材质分析</h1>
            <p className="text-muted-foreground">分析室内照片中的家具材质，生成材质指引图与性能对比表</p>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            <div className="lg:col-span-2 space-y-6">
              <Card className="p-6">
                ...
                <UploadZone value={imageUrl} onChange={(url) => { setImageUrl(url); reset() }} label="上传室内场景照片" hint="建议家具清晰可见的照片" />
              </Card>

              <Card className="p-6">
                <h3 className="font-semibold mb-4">生成配置</h3>
                <MaterialConfigPanel value={config} onChange={setConfig} />
              </Card>
            ...
            <div className="lg:col-span-3">
              <Card className="p-6">
                ...
                <ResultDisplay status={status} originalImage={imageUrl} resultImage={resultImage} error={error} onRetry={handleGenerate} onDownload={() => { if (resultImage) { const a = document.createElement("a"); a.href = resultImage; a.download = `banana-material-${Date.now()}.png`; a.target = "_blank"; a.click(); } }} />
              </Card>
            </div>
          </div>
        </div>
